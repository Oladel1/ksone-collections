...
@section('title', 'KS-One ' . $category->name . ' — Handcrafted in Nigeria')
...
<section id="home" class="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
    ...
    <div class="relative z-10 text-center px-6 max-w-3xl mx-auto">
        ...
        {{-- Subtitle — use category description if available --}}
        <p class="animate-fade-in-up ad-3
                  text-[#555] text-base sm:text-lg max-w-lg mx-auto mt-7 leading-relaxed">
            {{ $category->description ?? $s['hero_subtitle'] ?? 'Premium handcrafted goods designed for comfort, built with pride.' }}
        </p>
        ...
        <div class="animate-fade-in-up ad-4 flex flex-col sm:flex-row items-center justify-center gap-4 mt-10">
            ...
            <a href="https://wa.me/{{ $whatsapp }}?text={{ urlencode('Hello KS-One! I\'d like to order from your ' . $category->name . ' collection.') }}"
               target="_blank"
    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="text-center mb-14 scroll-reveal">
            ...
            <p class="text-[#555] text-base sm:text-lg max-w-md mx-auto mt-4 leading-relaxed">
                {{ $category->subtitle ?? $s['shop_subtitle'] ?? 'Handcrafted with premium materials for quality and style.' }}
            </p>
        </div>

        {{-- Filter Tabs (dynamic from product types) --}}
        @if (count($types) > 2)
        <div class="flex flex-wrap items-center justify-center gap-3 mb-12 scroll-reveal">
            @foreach ($types as $type)
                <button data-filter="{{ $type }}"
                        class="filter-btn {{ $type === 'all' ? 'active bg-[#1a1a1a] text-white' : 'bg-white text-[#555] border border-black/[0.1] hover:border-brand-500 hover:text-brand-500' }}
                               px-6 py-2.5 rounded-full text-sm font-semibold tracking-wider uppercase transition-all duration-300">
                    {{ $type === 'all' ? 'All' : ucfirst($type) . 's' }}
                </button>
            @endforeach
        </div>
        @endif
        ...
        @else
            ...
            <div class="text-center py-20">
                ...
                <p class="text-gray-500 mt-2 max-w-sm mx-auto">
                    We're working on adding products to our {{ $category->name }} collection. Check back soon!
                </p>
                <a href="{{ route('intro') }}"
                   class="inline-flex items-center gap-2 mt-6 px-6 py-3 bg-[#1a1a1a] text-white text-sm font-semibold rounded-full hover:bg-black transition-colors">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div class="scroll-reveal">
                ...
                <p class="text-[#555] text-base sm:text-lg mt-6 leading-relaxed">
                    {{ $s['about_text_1'] ?? 'KS-one is a proudly Nigerian brand dedicated to crafting premium leather goods that combine style, comfort, and durability.' }}
                </p>
                <p class="text-[#555] text-base sm:text-lg mt-4 leading-relaxed">
                    {{ $s['about_text_2'] ?? 'Every item is meticulously handcrafted by skilled artisans using the finest local materials — because we believe world-class goods should come from home.' }}
                </p>
